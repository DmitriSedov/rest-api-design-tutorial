...
module.exports = function validateNote( req, res, next ) {
  ...
  // the user maybe authenticated but check whether the user is authorized to access this note.
  const isAuthorized = usersCtrl.canUserAccessNote( user, req.params.id );
  // if the note does not exist or the user is not authorized to access the note, then throw `404 Not Found`.
